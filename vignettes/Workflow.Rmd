...
functions in the as-yet-unreleased [chipseq]{.sans-serif} package, which
provides convenient interfaces to other powerful packages such as
[ShortRead]{.sans-serif} and [IRanges]{.sans-serif}. We will also use
the [lattice]{.sans-serif} and [rtracklayer]{.sans-serif} packages for
visualization.
...
The `cstest` data set is included in the [chipseq]{.sans-serif} package
...
[ShortRead]{.sans-serif}, in conjunction with a filter produced by the
...
The above step has been performed in advance,
and the output has been included as a dataset in this package. We load
it now: 
```{r} 
data(cstest)
cstest 
...
## code used to convert the GenomeDataList to a
GRangesList cstest <- GenomicRangesList(lapply(cstest, function(gd)
...
```{r fig=TRUE,height=10,echo=FALSE}
...
```{r fig=TRUE,height=8,echo=FALSE}
plot(trellis.last.object()) 
```
...
```
```{r fig=TRUE,height=5,echo=FALSE}
plot(trellis.last.object())
```
...
The [GenomicFeatures]{.sans-serif} package facilitates obtaining gene
...
peakSummary$inGene <- peakSummary 
```
...
Using the [rtracklayer]{.sans-serif} package, we can upload our coverage
